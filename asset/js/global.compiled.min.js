...
console.log('media/js/jquery-3.3.1.min.js');
...
console.log('media/js/bootstrap.min.js');
...
//>> media/js/jquery.slicknav.js
console.log('media/js/jquery.slicknav.js');
...
console.log('media/js/owl.carousel.min.js');
 ...
 */
$.orchestrator={debug:!1,log:function(){if($.orchestrator.debug&&arguments.length>0){console.log("orchestrator >> "+arguments[0]);for(var o=1;o<arguments.length;o++)console.log(arguments[o])}},locks:{},reset:function(){var o=[];for(var r in $.orchestrator.locks)$.orchestrator.locks[r].immortal||o.push(r);for(var t=0;t<o.length;t++){var e=o[t];delete $.orchestrator.locks[e]}},resetLock:function(o){delete $.orchestrator.locks[o]},getLock:function(o){return o in $.orchestrator.locks?$.orchestrator.locks[o].lock:null},createLock:function(o,r){var t=$.Deferred();if(void 0!==o){if(o in $.orchestrator.locks){var e="lock "+o+" already exist";throw console.log("!> "+e),e}void 0===r&&(r=!1),$.orchestrator.locks[o]={lock:t,immortal:r}}return t},gotLock:function(o,r){if($.orchestrator.log("gotLock",o,r),void 0!==o){var t=$.orchestrator.getLock(o);return null!=t?(void 0!==r&&1==r&&($.orchestrator.locks[o].immortal=!0),t):$.orchestrator.createLock(o,r)}return $.orchestrator.createLock()},_getLockFrom:function(o,r){if("string"==typeof o)r.push($.orchestrator.gotLock(o));else if(o instanceof Array)for(var t=0;t<o.length;t++)$.orchestrator._getLockFrom(o[t],r);else r.push(o)},sync:function(o,r,t){var e=[];if($.orchestrator._getLockFrom(o,e),0==e.length)throw"no locks definitions found [$.orchestrator.sync]";$.when.apply($,e).then(r,t)}};;

//>> media/js/d3.V5.min.js
console.log('media/js/d3.V5.min.js');
...
//>> media/js/reseau.js
console.log('media/js/reseau.js');

class reseau{constructor(t){var e,n,a,i,r,o,d,s,c,l,u=this;function h(t,e){return t==e||s[t+"-"+e]}function f(){i.call(k),a.call(x),c.alphaTarget(.3).restart(),r.each((function(t,e){if(e%2==0)t.x=t.node.x,t.y=t.node.y;else{var n=this.getBBox(),a=t.x-t.node.x,i=t.y-t.node.y,r=Math.sqrt(a*a+i*i),o=n.width*(a-r)/(2*r);o=Math.max(-n.width,Math.min(0,o));this.setAttribute("transform","translate("+o+",16)")}})),r.call(k)}function g(t){return isFinite(t)?t:0}function p(t){var e=d3.select(d3.event.target).datum().index;i.style("opacity",(function(t){return h(e,t.index)?1:.1})),r.attr("display",(function(t){return h(e,t.node.index)?"block":"none"})),a.style("opacity",(function(t){return t.source.index==e||t.target.index==e?1:.1}))}function y(){r.attr("display","block"),i.style("opacity",1),a.style("opacity",1)}function x(t){t.attr("x1",(function(t){return g(t.source.x)})).attr("y1",(function(t){return g(t.source.y)})).attr("x2",(function(t){return g(t.target.x)})).attr("y2",(function(t){return g(t.target.y)}))}function k(t){t.attr("transform",(function(t){return"translate("+g(t.x)+","+g(t.y)+")"}))}function m(t){d3.event.sourceEvent.stopPropagation(),d3.event.active||l.alphaTarget(.3).restart(),t.fx=t.x,t.fy=t.y}function v(t){t.fx=d3.event.x,t.fy=d3.event.y}function w(t){d3.event.active||l.alphaTarget(0),t.fx=null,t.fy=null}this.cont=d3.select("#"+t.idCont),this.width=t.width?t.width:400,this.height=t.height?t.height:400,this.dataUrl=!!t.dataUrl&&t.dataUrl,this.kId=t.kId?t.kId:"o:id",this.kName=t.kName?t.kName:"o:title",this.data=t.data?t.data:{nodes:[{id:"Myriel",group:1},{id:"Napoleon",group:1}],links:[{source:"Napoleon",target:"Myriel",value:1},{source:"Mlle.Baptistine",target:"Myriel",value:8}]},this.init=function(){o=d3.scaleOrdinal(d3.schemeCategory10),d={nodes:[],links:[]},u.data.nodes.forEach((function(t,e){d.nodes.push({node:t}),d.nodes.push({node:t}),d.links.push({source:2*e,target:2*e+1})})),c=d3.forceSimulation(d.nodes).force("charge",d3.forceManyBody().strength(-50)).force("link",d3.forceLink(d.links).distance(0).strength(2)),l=d3.forceSimulation(u.data.nodes).force("charge",d3.forceManyBody().strength(-3e3)).force("center",d3.forceCenter(u.width/2,u.height/2)).force("x",d3.forceX(u.width/2).strength(1)).force("y",d3.forceY(u.height/2).strength(1)).force("link",d3.forceLink(u.data.links).id(t=>t[this.kId]).distance(50).strength(1)).on("tick",f),s=[],u.data.links.forEach((function(t){s[t.source.index+"-"+t.target.index]=!0,s[t.target.index+"-"+t.source.index]=!0})),e=this.cont.append("svg").attr("width",u.width+"px").attr("height",u.height+"px"),n=e.append("g"),e.call(d3.zoom().scaleExtent([.1,4]).on("zoom",(function(){n.attr("transform",d3.event.transform)}))),a=n.append("g").attr("class","links").selectAll("line").data(u.data.links).enter().append("line").attr("stroke","#aaa").attr("stroke-width","1px"),(i=n.append("g").attr("class","nodes").selectAll("g").data(u.data.nodes).enter().append("circle").attr("r",(function(t){return t.size?5*t.size:5})).attr("fill",(function(t){return o(t.group)}))).on("mouseover",p).on("mouseout",y),i.call(d3.drag().on("start",m).on("drag",v).on("end",w)),r=n.append("g").attr("class","labelNodes").selectAll("text").data(d.nodes).enter().append("text").text((function(t,e){let n=t.node[u.kName]?t.node[u.kName]:t.node[u.kId];return e%2==0?"":n})).style("fill","#555").style("font-family","Arial").style("font-size",12).style("pointer-events","none"),i.on("mouseover",p).on("mouseout",y)},this.hide=function(){e.attr("visibility","hidden")},this.show=function(){e.attr("visibility","visible")},u.dataUrl?d3.json(u.dataUrl).then((function(t){u.data=t,u.init()})):u.init()}};

//>> media/js/menuSunburst.js
console.log('media/js/menuSunburst.js');

var menu=null;class menuSunburst{constructor(t){this.titles=null,menu=this;var e=this;if(this.cont=d3.select("#"+t.idCont),this.width=t.width?t.width:400,this.callback=t.callback?t.callback:null,!t.data)throw new Exception("no data for sunburst");this.data=t.data;var a,n,r,i,l,s,o,d,c=d3.format(",d"),h=this.width/6,u=d3.arc().startAngle(t=>t.x0).endAngle(t=>t.x1).padAngle(t=>Math.min((t.x1-t.x0)/2,.005)).padRadius(1.5*h).innerRadius(t=>t.y0*h).outerRadius(t=>Math.max(t.y0*h,t.y1*h-1));function p(t){if(t.data.fct&&function(t){switch(t.data.fct){case"showRoueEmotions":e.hide(),n?n.show():n=new emotionswheel({idCont:e.cont.attr("id"),width:e.width,height:e.width});break;default:console.log(t)}}(t),menu.callback&&menu.callback(t,$(this)),!t.children)return;r.datum(t.parent||i),i.each(e=>e.target={x0:2*Math.max(0,Math.min(1,(e.x0-t.x0)/(t.x1-t.x0)))*Math.PI,x1:2*Math.max(0,Math.min(1,(e.x1-t.x0)/(t.x1-t.x0)))*Math.PI,y0:Math.max(0,e.y0-t.depth),y1:Math.max(0,e.y1-t.depth)});const a=s.transition().duration(750);o.transition(a).tween("data",t=>{const e=d3.interpolate(t.current,t.target);return a=>t.current=e(a)}).filter((function(t){return+this.getAttribute("fill-opacity")||x(t.target)})).attr("fill-opacity",t=>x(t.target)?t.children?1:.8:0).attrTween("d",t=>()=>u(t.current)),d.filter((function(t){return+this.getAttribute("fill-opacity")||y(t.target)})).transition(a).attr("fill-opacity",t=>+y(t.target)).attrTween("transform",t=>()=>f(t.current))}function x(t){return t.y1<=3&&t.y0>=1&&t.x1>t.x0}function y(t){return t.y1<=3&&t.y0>=1&&(t.y1-t.y0)*(t.x1-t.x0)>.03}function f(t){const e=(t.x0+t.x1)/2*180/Math.PI;return`rotate(${e-90}) translate(${(t.y0+t.y1)/2*h},0) rotate(${e<180?0:180})`}this.init=function(){var t,n;console.log(e.data),t=e.data,i=d3.hierarchy(t).sum(t=>t.size).sort((t,e)=>e.value-t.value),i=d3.partition().size([2*Math.PI,i.height+1])(i),l=d3.scaleOrdinal().range(d3.quantize(d3.interpolateRainbow,e.data.children.length+1)),i.each(t=>t.current=t),a=e.cont.append("svg").style("width","96%").style("height","96%").style("position","absolute").attr("viewBox","0 0 "+e.width+" "+e.width).style("font","5px sans-serif"),s=a.append("g").attr("transform",`translate(${e.width/2},${e.width/2})`),(o=s.append("g").selectAll("path").data(i.descendants().slice(1)).join("path").attr("fill",t=>{for(;t.depth>1;)t=t.parent;return l(t.data.name)}).attr("fill-opacity",t=>x(t.current)?t.children?1:.8:0).attr("d",t=>u(t.current))).style("cursor","pointer").on("click",p),o.append("title").text(t=>`${t.ancestors().map(t=>t.data.name).reverse().join("/")}\n${c(t.value)}`),d=s.append("g").attr("pointer-events","none").attr("text-anchor","middle").style("user-select","none").selectAll("text").data(i.descendants().slice(1)).join("text").attr("dy","0.35em").attr("fill-opacity",t=>+y(t.current)).attr("transform",t=>f(t.current)).text(t=>t.data.name),n=30,d.each((function(){for(var t,e=d3.select(this),a=e.text().split(/\s+/).reverse(),r=[],i=1,l=e.attr("y"),s=parseFloat(e.attr("dy")),o=e.text(null).append("tspan").attr("x",0).attr("y",l).attr("dy",s+"em"),d=o;t=a.pop();)r.push(t),o.text(r.join(" ")),o.node().getComputedTextLength()>n&&(1==r.length?(o.text(r.join(" ")),r=[],a.length>0&&(o=e.append("tspan").attr("x",0).attr("y",l).attr("dy","1em"),i++)):(r.pop(),o.text(r.join(" ")),r=[t],o=e.append("tspan").attr("x",0).attr("y",l).attr("dy","1em").text(t),i++));d.attr("dy",-.5*(i-1)+.25+"em")})),r=s.append("circle").datum(i).attr("r",h).attr("fill","none").attr("pointer-events","all").on("click",p)},this.hide=function(){a.attr("visibility","hidden")},this.show=function(){a.attr("visibility","visible"),n&&n.hide()},this.init()}};

//>> media/js/geoLoc.js
console.log('media/js/geoLoc.js');

class Geo{constructor(){this.enabled=!1,"geolocation"in navigator&&(this.enabled=!0)}getPosition(o){this.enabled?navigator.geolocation.getCurrentPosition((function(n){o(n.coords)})):o(null)}};

//>> media/js/proto.js
console.log('media/js/proto.js');

class Proto{constructor(e){this.useLocal=e||!1,this.useCorsProxy=this.useLocal,this.geo=new Geo,this.geo.getPosition((function(e){})),this.resultData={resource:{type:null,value:null},property:[],user:null,geoLoc:null,date:null,info:null},this.circularMenuData=null}getGetParameters(){var e=location.search.substring(1);return e?JSON.parse('{"'+decodeURI(e).replace(/"/g,'\\"').replace(/&/g,'","').replace(/=/g,'":"')+'"}'):{}}getSyncJson(e){this.useCorsProxy&&!e.startsWith("http://127.0.0.1:5000")&&(e="https://cors-anywhere.herokuapp.com/"+e);var t=null;return $.ajax({type:"GET",url:e,dataType:"json",success:function(e){t=e},data:{},async:!1}),t}getInformationsData(){var e=null;return e=this.useLocal?"http://127.0.0.1:5000/media/data/dataLocalInformations.json":"http://polemika.univ-paris8.fr/omk/api/items?item_set_id=2",this.getSyncJson(e)}getOneInformationData(e){var t=this.getInformationsData(),a=null;if(this.useLocal)a=t[0];else if(null==e){var r=Math.floor(Math.random()*(t.length-1));a=t[r]}else $.each(t,(function(t,r){if(r["o:id"]==e)return a=r,!1}));return a}getMenuData(e){var t=null;t=this.useLocal?"http://127.0.0.1:5000/media/data/dataMenuCirculaire.json":"http://polemika.univ-paris8.fr/omk/api/items?resource_class_id=133",$.getJSON(t,e)}getMediaData(e){return this.useLocal&&(e="http://127.0.0.1:5000/media/data/dataLocalMedia.json"),this.getSyncJson(e)}readInformation(e){var t=this,a=[],r=a,o=function(e){if(Array.isArray(e))$.each(e,(function(e,t){o(t)}));else{var a=e["@type"];if(a&&a.includes("o:Item")){var s={texte:e["o:title"],omkId:{id:e["o:id"],type:a},media:null},i=e["o:media"][0]["@id"],l=t.getMediaData(i),n={imgUrl:l["o:source"],omkId:{id:l["o:id"],type:l["@type"]}};s.media=n,r.push(s)}}};return o(e),a[0]}getOneInformation(e){var t=this.getOneInformationData(e);return this.readInformation(t)}getCircularMenuData(e){this.circularMenuData?e(this.circularMenuData):this.getMenuData((function(t){var a={name:"menu",color:"magenta",children:[]},r=a.children,o=function(e){if(Array.isArray(e))$.each(e,(function(e,t){o(t)}));else if((s=e["@type"])&&s.includes("o:Item")&&s.includes("plmk:Monde")){var t={name:e["dcterms:title"][0]["@value"],color:"red",omkId:{id:e["o:id"],type:s},children:[]};r.push(t);var a=r;r=t.children,$.each(e,(function(e,t){if(e.startsWith("plmk:")&&t.length>0){var a={name:t[0].property_label,color:"orange",omkId:{id:t[0].property_id,type:t[0].type},children:[]};r.push(a);var s=r;r=a.children,o(t),r=s}})),r=a}else{var s;if((s=e.type)&&"resource"==s){var i={name:e.display_title,color:"blue",size:1,omkId:{id:e.value_resource_id,type:e.value_resource_name}};r.push(i)}}};o(t),this.circularMenuData=a,e(a)}))}getResult(){return this.resultData}serializeResult(e){var t=this;t.geo.getPosition((function(a){t.resultData.geoLoc=a;var r=new Date;t.resultData.date=r.getDate()+"-"+(r.getMonth()+1)+"-"+r.getFullYear(),"picture"==t.resultData.resource.type?t.resultData.resource.omkId=t.resultData.info.media.omkId:t.resultData.resource.omkId=t.resultData.info.omkId;var o={"dcterms:title":"TODO","jdc:creationDate":t.resultData.date,"ma:locationLatitude":a.latitude,"ma:locationLongitude":a.longitude,"oa:hasSource":[{type:"resource",value:t.resultData.resource}],"TD:property":t.resultData.property,"TD:user":"TODO","TD:info":t.resultData.info.omkId};e(o)}))}sendResult(){console.log("SEND DATA"),console.log(self.resultData)}selectInfo(e,t){e.hasClass("infoText")?this.getResult().resource.type="picture":(this.getResult().resource.type="text",this.getResult().resource.value=t),$(".result").text(""),$(".boutonQualifier").removeClass("hidden"),$(".infoElt").removeClass("selected"),e.addClass("selected")}deselectInfo(){$(".boutonQualifier").addClass("hidden"),$(".infoElt").removeClass("selected")}};
